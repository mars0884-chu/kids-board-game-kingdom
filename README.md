...
這是提供 7 歲兒童使用的離線優先 PWA 棋藝遊戲。v0.9.4 承接已由製作人確認合格的 v0.7.3 跳棋與 v0.9.3 WebRTC 直連驗證，改用乾淨正面 3D 棋棋貓頭像製作手機主畫面圖示，並加入只協助探索連線候選的公開 STUN；不建立本專案伺服器、房間或棋局儲存。甲分享邀請連結，乙開啟後自動產生回覆連結並只按一次「一鍵回覆給甲」，甲開啟回覆連結後自動連線。目前 WebRTC 僅整合跳棋；其他棋類尚未接入連線，原有離線／同機雙人模式不受影響。跳棋仍是標準 121 孔六角星棋盤、雙方各 10 枚、一般移動／連跳、目標營陣鎖定、六關差異化互動教學、四階兒童友善自適應難度 NPC、同機雙人與 IndexedDB 存檔；不設定人工回合上限。所有兒童文案維持台灣繁體中文、每字右側直排台灣注音與語音欄位。
...
本版不建立雲端儲存或伺服器，也不建立帳號、房間資料庫、QR Code、TURN 中繼、第三方額度或兒童個人資料。雙裝置連線只在兩台裝置間使用 WebRTC data channel；公開 STUN 只協助瀏覽器探索可用連線路徑，不保存棋局、不傳送棋步、不建立房間。邀請／回覆 URL 只短期攜帶連線資料，甲原本的遊戲分頁必須保持開啟。STUN 可改善部分 NAT／行動網路連線，但仍可能受防火牆、瀏覽器或網路政策限制；失聯時保留本機最後局面，但需要重新交換連結。完整範圍與限制見 `docs/P09_ONLINE_SPEC.md`。

## 開啟完整驗證

1. 要直接遊玩請開啟上方 GitHub Pages 網址；本機驗證時請在專案工作區或完整驗證封包中啟動伺服器，不要直接開啟 `index.html`。
2. 目前工作區先以 `npm.cmd run dev -- --host 127.0.0.1 --port 5173` 啟動 v0.9.4，或在首頁依序選「雙裝置連線」→「跳棋」。跳棋直接預覽可使用 `?preview=jump-chess-adventure`、`?preview=jump-chess`、`?preview=jump-chess-local`、`?preview=jump-chess-online`；完整驗證封包位於 `releases/kids-board-game-kingdom-v0.9.4.zip`。
