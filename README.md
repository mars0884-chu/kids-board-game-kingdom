...
這是提供 7 歲兒童使用的離線優先 PWA 棋藝遊戲。v0.9.2 承接已由製作人確認合格的 v0.7.3 跳棋與 v0.9.1 WebRTC 直連驗證，修正 iPhone Safari／PWA 的逐字注音排版、手機頁面捲動與主畫面圖示；仍不使用本專案伺服器／雲端。甲分享邀請連結，乙開啟後自動產生回覆連結並只按一次「一鍵回覆給甲」，甲開啟回覆連結後自動連線。目前 WebRTC 僅整合跳棋；其他棋類尚未接入連線，原有離線／同機雙人模式不受影響。跳棋仍是標準 121 孔六角星棋盤、雙方各 10 枚、一般移動／連跳、目標營陣鎖定、六關差異化互動教學、四階兒童友善自適應難度 NPC、同機雙人與 IndexedDB 存檔；不設定人工回合上限。所有兒童文案維持台灣繁體中文、每字右側直排台灣注音與語音欄位。
...
2. 目前工作區先以 `npm.cmd run dev -- --host 127.0.0.1 --port 5173` 啟動 v0.9.2，或在首頁依序選「雙裝置連線」→「跳棋」。跳棋直接預覽可使用 `?preview=jump-chess-adventure`、`?preview=jump-chess`、`?preview=jump-chess-local`、`?preview=jump-chess-online`；完整驗證封包位於 `releases/kids-board-game-kingdom-v0.9.2.zip`。
