...
這是提供 7 歲兒童使用的離線優先 PWA 棋藝遊戲。v0.9.8 承接已由製作人確認合格的 v0.7.3 跳棋、v0.9.3 WebRTC 直連與 v0.9.4 公開 STUN／正面棋棋貓圖示，修正 Safari 雙方就緒競態後，補上斷線時棋孔原生鎖定與回歸測試。不建立本專案伺服器、房間或棋局儲存。甲分享邀請連結，乙開啟後自動產生回覆連結並只按一次「一鍵回覆給甲」，甲開啟回覆連結後自動連線；只有雙方都回報同一連線已準備好，兩端才會進入棋盤。目前 WebRTC 僅整合跳棋；其他棋類尚未接入連線，原有離線／同機雙人模式不受影響。跳棋仍是標準 121 孔六角星棋盤、雙方各 10 枚、一般移動／連跳、目標營陣鎖定、六關差異化互動教學、四階兒童友善自適應難度 NPC、同機雙人與 IndexedDB 存檔；不設定人工回合上限。所有兒童文案維持台灣繁體中文、每字右側直排台灣注音與語音欄位。
...
2. 目前工作區先以 `npm.cmd run dev -- --host 127.0.0.1 --port 5173` 啟動 v0.9.8，或在首頁依序選「雙裝置連線」→「跳棋」。跳棋直接預覽可使用 `?preview=jump-chess-adventure`、`?preview=jump-chess`、`?preview=jump-chess-local`、`?preview=jump-chess-online`；完整驗證封包位於 `releases/kids-board-game-kingdom-v0.9.8.zip`。
