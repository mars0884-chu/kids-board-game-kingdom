...
這是提供 7 歲兒童使用的離線優先 PWA 棋藝遊戲。v0.9.3 承接已由製作人確認合格的 v0.7.3 跳棋與 v0.9.2 WebRTC 直連驗證，提升 iPhone Safari／PWA 主畫面圖示的光影層次、徽章質感與縮小辨識度；仍不使用本專案伺服器／雲端。甲分享邀請連結，乙開啟後自動產生回覆連結並只按一次「一鍵回覆給甲」，甲開啟回覆連結後自動連線。目前 WebRTC 僅整合跳棋；其他棋類尚未接入連線，原有離線／同機雙人模式不受影響。跳棋仍是標準 121 孔六角星棋盤、雙方各 10 枚、一般移動／連跳、目標營陣鎖定、六關差異化互動教學、四階兒童友善自適應難度 NPC、同機雙人與 IndexedDB 存檔；不設定人工回合上限。所有兒童文案維持台灣繁體中文、每字右側直排台灣注音與語音欄位。

本版跳棋畫面沿用已核准的正式美術／版面方向；連線頁按鍵沿用共用按鍵的配色、圖示與漢字／右側直排注音契約，並以四個指定尺寸與 iPhone 15 Pro Max 430×932 比例確認不溢出。公開儲存庫只保留可執行原始碼、必要測試／規格、部署設定與已核准遊戲素材；完整驗證封包與歷史封存仍由製作工作區獨立保存。
...
2. 目前工作區先以 `npm.cmd run dev -- --host 127.0.0.1 --port 5173` 啟動 v0.9.3，或在首頁依序選「雙裝置連線」→「跳棋」。跳棋直接預覽可使用 `?preview=jump-chess-adventure`、`?preview=jump-chess`、`?preview=jump-chess-local`、`?preview=jump-chess-online`；完整驗證封包位於 `releases/kids-board-game-kingdom-v0.9.3.zip`。
...
連線頁版面回歸可執行 `npm.cmd run verify:jump-chess-online-layout`；它會以 Edge DPR 1 實際檢查四個正式尺寸，以及 430×932／932×430 的手機比例頁面與按鍵內文邊界。
