...
v0.13.3 增加供手機實機試看的象棋技術測試入口：`?preview=xiangqi-art-proposal`。此入口未列入一般遊戲選單；象棋美術、台灣口音語音、完整長將／長捉判決與兒童觸控驗收尚未完成，不代表正式版核准。完整來源與測試隨本版推送。

v0.13.2 僅同步八碼房號的 Firebase 整合測試與六款遊戲瀏覽器驗證，不再變更 v0.13.1 的玩法；完整封包與公開網站請以此版為準。
...
2. 目前工作區先以 `npm.cmd run dev -- --host 127.0.0.1 --port 5173` 啟動 v0.10.6，或在首頁依序選「雙裝置連線」→「跳棋」。跳棋直接預覽可使用 `?preview=jump-chess-adventure`、`?preview=jump-chess`、`?preview=jump-chess-local`、`?preview=jump-chess-online`；完整驗證封包位於 `releases/kids-board-game-kingdom-v0.10.6.zip`。
